package com.archclient.client.util.title;
...
public class TitleManager {
    ...
    public void onDraw(GuiDrawEvent event) {
        ...
        for (Title title : this.titles) {
            ...
            this.mc.fontRendererObj.drawStringWithShadow(Title.getMessage(title), (int)(((float)(event.getResolution().getScaledWidth() / 2) / f) - (this.mc.fontRendererObj.getStringWidth(Title.getMessage(title)) / 2f)), (int)(((float)(event.getResolution().getScaledHeight() / 2 - 9 / 2) + f2) / f), new Color(1.0f, 1.0f, 1.0f, f3).getRGB());
