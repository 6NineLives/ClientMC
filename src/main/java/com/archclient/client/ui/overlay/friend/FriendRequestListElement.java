...
import com.archclient.client.websocket.client.WSPacketClientRequestsStatus;
import com.archclient.client.websocket.shared.WSPacketFriendRequest;
...
public class FriendRequestListElement extends ElementListElement<FriendRequestElement> {
    ...
    public void handleElementDraw(float mouseX, float mouseY, boolean enableMouse) {
        ...
        if (!ArchClient.getInstance().getAssetsWebSocket().isOpen()) {
            ...
            this.toggleRequests.render((ArchClient.getInstance().isAcceptingFriendRequests() ? "Disable" : "Enable") + " incoming friend requests", mouseX, mouseY, true);
    ...
    public boolean handleElementMouseClicked(float mouseX, float mouseY, int mouseButton, boolean enableMouse) {
        ...
        if (this.toggleRequests.isMouseInside(mouseX, mouseY - this.scrollableElement.getTranslateY())) {
            ArchClient.getInstance().getAssetsWebSocket().sendToServer(new WSPacketClientRequestsStatus(!ArchClient.getInstance().isAcceptingFriendRequests()));
            ArchClient.getInstance().setAcceptingFriendRequests(!ArchClient.getInstance().isAcceptingFriendRequests());
            return false;
        }
        boolean bl2 = false;
    ...
    private void lIIlIlIllIIlIIIlIIIlllIII() {
        if (!this.username.getText().isEmpty()) {
            ...
            if (string.matches("([a-zA-Z0-9_]+)") && string.length() <= 16) {
                ArchClient.getInstance().getAssetsWebSocket().sendToServer(new WSPacketFriendRequest("", this.username.getText()));
