...
public class ModuleSettingsElement extends AbstractModulesGuiElement {
    ...
    public void handleDrawElement(int mouseX, int mouseY, float partialTicks) {
        ...
        int n3 = 75;
        Ref.modified$drawRect(this.x, this.y + this.height - 1, this.x + this.width, this.y + this.height, ArchClient.getInstance().globalSettings.isDarkMode() ? -14211288 : 791621423);
        ...
        FontRegistry.getPlayBold18px().drawString(this.module.getName().toUpperCase(), (float)this.x + (float)14, (float)this.y + (float)3, this.parent.lIIIIlIIllIIlIIlIIIlIIllI(this.module) ? (ArchClient.getInstance().globalSettings.isDarkMode() ? -1 : -818991313) : 0x2F2F2F2F);
    }
