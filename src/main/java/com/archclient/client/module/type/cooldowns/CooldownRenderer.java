...
public class CooldownRenderer {
    ...
    public void lIIIIlIIllIIlIIlIIIlIIllI(Setting setting, float f, float f2, int n) {
        float f3;
        int n2 = 17;
        Ref.getGlBridge().bridge$pushMatrix();
        float f4 = ArmourStatusModule.renderItem.getZLevel();
        ArmourStatusModule.renderItem.setZLevel(-200);
        float f5 = 1.35f;
        Ref.getGlBridge().bridge$translate(-0.5f, -1, 0.0f);
        Ref.getGlBridge().bridge$scale(f5, f5, f5);
        RenderHelper.enableStandardItemLighting();
        ArmourStatusModule.renderItem.renderItemAndEffectIntoGUI(this.item, (int)((f + (float)(n2 / 2)) / f5), (int)((f2 + (float)(n2 / 2)) / f5));
        RenderHelper.disableStandardItemLighting();
        Ref.getGlBridge().bridge$popMatrix();
        ArmourStatusModule.renderItem.setZLevel(f4);
        double d = this.duration - (System.currentTimeMillis() - this.time);
        if (d <= 0.0) {
            return;
        }
        if (setting.<String>value().equalsIgnoreCase("Bright")) {
        ...
        } else if (setting.<String>value().equalsIgnoreCase("Dark")) {
            Ref.getGlBridge().bridge$color(0f, 0f, 0f, 0.2f);
        ...
        } else if (setting.<String>value().equalsIgnoreCase("Colored")) {
